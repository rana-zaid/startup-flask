...
def allStartups():
    ...
    if 'username' not in login_session:
        return render_template('publicStartups.html', startups=startups)
    else:
...
def startupDetails(startup_id):
    ...
    creator = getUserInfo(startup.user_id)
    founders = session.query(Founder).filter_by(startup_id=startup_id).all()
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('public_startup_details.html' ,startup=startup, founders=founders, creator=creator)
    else:
